...
class MyTextEditingController extends TextEditingController {
  ...
  MyTextEditingController.fromValue(
  ...
  int get selectionStart =>
      this.selection.start > -1 ? this.selection.start : this.text.length;

  int get selectionEnd =>
      this.selection.end > -1 ? this.selection.end : this.text.length;
  ...
  void insertSpanInfo(SuperTextInfo info) {
    ...
    TextSelection newSelection =
        new TextSelection.collapsed(offset: indertCursor + info.length);
  ...
  deleteSpanInfo() {
    ...
    TextSelection newSelection;
    if (acc - 1 < 0) {
      newText = this.text.substring(0, indertCursor - 1) +
          this.text.substring(indertCursor);
      newSelection = new TextSelection.collapsed(offset: indertCursor - 1);
    } else {
      SuperTextInfo info = _spanInfos[acc - 1];
      String compare = text.substring(indertCursor - 1, indertCursor);
      if (info.toString().endsWith(compare)) {
        _spanInfos.remove(info);
        newText = this.text.substring(0, indertCursor - info.length) +
            this.text.substring(indertCursor);
        newSelection =
            new TextSelection.collapsed(offset: indertCursor - info.length);
      } else {
  ...
  TextSpan _buildTextSpan(SuperTextInfo info) {
    if (info is TextInfo) {
      ...
      return new TextSpan(text: info.toString(), style: this.atStyle);
    } else if (info is TopicInfo) {
      return new TextSpan(text: info.toString(), style: this.topicStyle);
  ...
  TextSelection adjustSelectWord(TextPosition position) {
    ...
    if (end != -1) {
      return new TextSelection(baseOffset: i, extentOffset: end);
    }
    return null;
  }
  ...
  TextEditingValue adjustTextChange(String _oldText, TextEditingValue value) {
    ...
    if (_newText.contains(_oldText) &&
        (_newSelection.end == _newText.length ||
            _newSelection.end == _newText.length - _oldText.length)) {
      return value;
    }
    var spans = toInfos(_oldText);
    if (_oldSelection.isCollapsed &&
        _oldSelection.start == _newSelection.start + 1) {
      _oldSelection = new TextSelection(
          extentOffset: _oldSelection.end, baseOffset: _oldSelection.start - 1);
    }
    int left = _oldSelection.start;
    int right = _oldSelection.end;
    var posLeft = _getPos(spans, _oldSelection.start,
        rightBorder: false, leftBorder: true);
    if (!(posLeft.span is TextInfo)) {
      left = posLeft.start;
    } else {}
    var posRight =
        _getPos(spans, _oldSelection.end, rightBorder: true, leftBorder: false);
    ...
    String addStr =
        _newText.substring(_oldSelection.start, _newSelection.start);
  ...
  _Pos _getPos(
    ...
    for (_span in spans) {
      ...
      bool rightBol =
          rightBorder ? (diff <= _span.length) : (diff < _span.length);
      if (leftBol && rightBol) {
        break;
      }
      if (!(_span is TextInfo)) {
    ...
    return _Pos(
        acc: acc, start: i, diff: diff, span: _span, end: i + _span.length);
  }
}

class _Pos {
  _Pos({this.acc, this.start, this.diff, this.span, this.end});
