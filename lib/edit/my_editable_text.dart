...
import 'my_text_editing_controller.dart';
...
/// MyEditableText receives focus. For example, providing iOS's "emergencyCall"
...
class MyEditableText extends StatefulWidget {
  ...
  ///
  /// See [MyRenderEditable.locale] for more information.
  final Locale locale;
  ...
  /// If true, the [MyRenderEditable] created by this widget will not handle
  /// pointer events, see [renderEditable] and [MyRenderEditable.ignorePointer].
  ///
...
/// State for a [MyEditableText].
class MyEditableTextState extends State<MyEditableText> with AutomaticKeepAliveClientMixin<MyEditableText>, WidgetsBindingObserver implements TextInputClient, TextSelectionDelegate {
  Timer _cursorTimer;
  ...
  void updateEditingValue(TextEditingValue value) {
    ...
    if (value.text != _value.text) {
      value = widget.controller.adjustTextChange(_value.text,value);
    }
  ...
  void performAction(TextInputAction action) {
    switch (action) {
      case TextInputAction.newline:
        // If this is a multiline MyEditableText, do nothing for a "newline"
  ...
  void _handleSelectionChanged(TextSelection selection, MyRenderEditable renderObject, SelectionChangedCause cause) {
    ...
    if (widget.selectionControls != null) {
      _selectionOverlay = MyTextSelectionOverlay(
        context: context,
        value: _value,
  ...
  @visibleForTesting
  MyTextSelectionOverlay get selectionOverlay => _selectionOverlay;
  ...
  void _didChangeTextEditingValue() {
    ...
    // TODO(abarth): Teach MyRenderEditable about ValueNotifier<TextEditingValue>
  ...
  /// when [ignorePointer] is true. See [MyRenderEditable.ignorePointer].
  MyRenderEditable get renderEditable => _editableKey.currentContext.findRenderObject();
      ...
      viewportBuilder: (BuildContext context, ViewportOffset offset) {
        return CompositedTransformTarget(
          ...
          child: Semantics(
            ...
            child: _Editable(
              key: _editableKey,
              cotroller: widget.controller,
              textSpan: buildTextSpan(),
              value: _value,
              cursorColor: widget.cursorColor,
              showCursor: MyEditableText.debugDeterministicCursor ? ValueNotifier<bool>(true) : _showCursor,
              hasFocus: _hasFocus,
  ...
  TextSpan buildTextSpan() {
    ...
    if (widget.obscureText) {
      text = MyRenderEditable.obscuringCharacter * text.length;
      final int o =
        _obscureShowCharTicksPending > 0 ? _obscureLatestCharIndex : null;
      if (o != null && o >= 0 && o < text.length)
        text = text.replaceRange(o, o + 1, _value.text.substring(o, o + 1));
    }
    var spans = widget.controller.toTextSpans();
    spans.forEach((span){
...
class _Editable extends LeafRenderObjectWidget {
  const _Editable({
    ...
    this.textSelectionDelegate,
       ...
       assert(enableInteractiveSelection != null),
       super(key: key);
  final MyTextEditingController cotroller;
  ...
  final MySelectionChangedHandler onSelectionChanged;
  ...
  @override
  MyRenderEditable createRenderObject(BuildContext context) {
    return _RichRenderEditable(
      cotroller: this.cotroller,
      text: textSpan,
  ...
  @override
  void updateRenderObject(BuildContext context, MyRenderEditable renderObject) {
    renderObject
      ..text = textSpan
      ..cursorColor = cursorColor
      ..showCursor = showCursor
...
class _RichRenderEditable extends MyRenderEditable{
  _RichRenderEditable({
    @required MyTextEditingController cotroller,
    TextSpan text,
    Color cursorColor,
    ...
    MySelectionChangedHandler onSelectionChanged,
  ...
  void paintImageSpan(PaintingContext context, Offset offset) {
    ...
    List<TextSpan> spans = text.children;
    ...
    int textOffset = 0;
    for (TextSpan textSpan in spans) {
      if (textSpan is ImageSpan) {
        // this is the top-center point of the ImageSpan
        Rect rect = getLocalRectForCaret(
          TextPosition(offset: textOffset),
        );
        Offset topLeft = rect.topLeft+offset;
        ...
        double fontSize = textSpan.style?.fontSize??text.style.fontSize??rect.height*0.857;
        
        Rect imgRect = topLeft & Size(fontSize, fontSize);
